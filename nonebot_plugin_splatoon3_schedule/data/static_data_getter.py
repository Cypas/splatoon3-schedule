    ...
    for weapon_info in weapon_list:
        ...
        for i in range(3):
            ...
            await get_image_info(
                ImageInfo(
                    name=names[i],
                    url=None,
                    source_type=weapon_image_type[i],
                    zh_name=None,
                )
            )  # 多余项忽略
        # 类型图片，没有找到 File 页面
        await push_weapon_images(
            ImageInfo(
                name=names[3],
                url="https:"
                + weapon_info[ids[3]].contents[0].contents[0].contents[0].attrs["src"],
...
async def get_image_info(imageInfo: ImageInfo):
    """网页爬取图片信息"""
    global base_url
    url = base_url + "/File:S3_Weapon_{}_{}.png".format(
        imageInfo.source_type, imageInfo.name.replace(" ", "_")
    )
